...
{% macro teradata__get_period_sql(target_cols_csv, sql, timestamp_field, period, start_timestamp, stop_timestamp, offset) -%}
  ...
  {%- set period_filter -%}
    ({{timestamp_field}} >  cast(cast('{{start_timestamp}}' as timestamp) + interval '{{offset}}' {{period}} as timestamp)) and
     ({{timestamp_field}} <= cast(cast('{{start_timestamp}}' as timestamp) + interval '{{offset}}' {{period}} + interval '1' {{period}} as timestamp)) and
